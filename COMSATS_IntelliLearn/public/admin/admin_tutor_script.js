...
toggle.onclick = function () {
    ...
    // document.querySelector(".navigation").classList.remove("resp-nav")
};
...
const search = document.getElementById("searchMG");
const userTable = document.getElementById("tutorsearch");
    ...
    fetch('/approve-tutors-details') // Replace this with your backend route
        ...
        .then(data => {
            ...
            data.forEach(tutor => {
                ...
                row.innerHTML = `
                    <td>${tutor.id}</td>
                    <td class="t_name">${tutor.name}</td>
                    <td class="t_email">${tutor.email}</td>
                    <td>${tutor.bio}</td>
                    <td>${tutor.birthday}</td>
                    <td>${tutor.country}</td>
                    <td class="t_no">${tutor.phone_no}</td>
...
const searchAp = document.getElementById("approvetutor");
const userTableAp = document.getElementById("approveSearch");
const rowsAp = userTableAp.getElementsByTagName("tr");

searchAp.addEventListener("input", function () {
    const searchTerm = searchAp.value.toLowerCase();

    for (let i = 1; i < rowsAp.length; i++) {
        const tutorName = rowsAp[i].querySelector('.t_name').textContent.toLowerCase();
        const tutorSubject = rowsAp[i].querySelector('.t_email').textContent.toLowerCase();
        const tutorEmail = rowsAp[i].querySelector('.t_no').textContent.toLowerCase();

        if (tutorName.includes(searchTerm) || tutorSubject.includes(searchTerm) || tutorEmail.includes(searchTerm)) {
            rowsAp[i].style.display = "";
        } else {
            rowsAp[i].style.display = "none";
        }
    }
});
